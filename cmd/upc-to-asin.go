...
type LookupAsinFromUpcConfig struct {
	...
	Single bool
	// may be a single UPC string or a path to the text file
...
var (
	lookupAsinFromUpcCmd = &cobra.Command{
		...
		Run:   WrapCommandWithResources(lookupAsinFromUpc, ResourceConfig{ResourceTypes: []ResourceType{ResourceAmazon}}),
	}
...
func getlookupAsinFromUpcCmd() *cobra.Command {
	...
	lookupAsinFromUpcCmd.PersistentFlags().StringVarP(&lookupAsinFromUpcConfig.UPC, "upc", "u", "", "newline delimited (one per line) text file that contains UPCs, or, a single UPC (if so, --single flag must be provided)")
...
func lookupAsinFromUpc(cmd *cobra.Command, args []string) {
	if lookupAsinFromUpcConfig.UPC == "" {
		cmd.Help()
	...
	if lookupAsinFromUpcConfig.Single {
		queryIdentifiers = append(queryIdentifiers, strings.TrimSpace(lookupAsinFromUpcConfig.UPC))
	} else {
		contents, err := os.ReadFile(lookupAsinFromUpcConfig.UPC)
		if err != nil {
			if os.IsNotExist(err) {
				log.Fatal().Err(err).Msgf("path %s does not exist", lookupAsinFromUpcConfig.UPC)
			}
	...
	params.SetIncludedData([]string{"identifiers", "attributes"})
	...
	if lookupAsinFromUpcConfig.Single {
		...
		log.Info().
			...
			Str("upc", lookupAsinFromUpcConfig.UPC).
			Msg("found!")
	} else {
